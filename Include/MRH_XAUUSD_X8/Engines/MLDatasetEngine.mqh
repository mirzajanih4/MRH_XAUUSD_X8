...
// STEP118 - Validation History Internal State
double m_validationHistorySum;
double m_validationHistoryMin;
double m_validationHistoryMax;
int    m_validationHistorySamples;
...
// STEP122 - Persistent Execution Bottleneck Statistics
int m_bottleneckStructureCount;
int m_bottleneckOBCount;
int m_bottleneckLiquidityCount;
int m_bottleneckPermissionCount;
int m_executionAllowedCount;
int m_bottleneckTotalSamples;

// STEP123 - Root Cause Analytics
int m_rootNoStructureBiasCount;
int m_rootStructureRangeCount;
int m_rootNoValidOBCount;
int m_rootOBInvalidatedCount;
...
public:
   CMLDatasetEngine()
   {
     ...
     m_validationHistorySum = 0.0;
m_validationHistoryMin = 0.0;
m_validationHistoryMax = 0.0;
m_validationHistorySamples = 0;

m_bottleneckStructureCount = 0;
m_bottleneckOBCount = 0;
m_bottleneckLiquidityCount = 0;
m_bottleneckPermissionCount = 0;
m_executionAllowedCount = 0;
m_bottleneckTotalSamples = 0;

m_rootNoStructureBiasCount = 0;
m_rootStructureRangeCount = 0;
m_rootNoValidOBCount = 0;
m_rootOBInvalidatedCount = 0;
...
row += "," + m_memory.LastSnapshot.TradeLifecycleClass;
   ...
   void BuildTradeSnapshot()
   {
      ...
      m_memory.LastSnapshot.OBScore =
         ...
         m_memory.LastSnapshot.OBValid =
   m_memory.OB.Valid;

m_memory.LastSnapshot.OBFreshness =
   m_memory.OB.Freshness;

m_memory.LastSnapshot.OBMitigated =
   m_memory.OB.Mitigated;

m_memory.LastSnapshot.OBInvalidated =
   m_memory.OB.Invalidated;

m_memory.LastSnapshot.PrimaryBlockReason =
   m_memory.Execution.PrimaryBlockReason;

// STEP129B - Entry Quality Learning Defaults
m_memory.LastSnapshot.EntryQualityScore = 0.0;
m_memory.LastSnapshot.EntryQualityClass = "ENTRY_QUALITY_NOT_EVALUATED";
m_memory.LastSnapshot.EntryQualityReason = "ENTRY_QUALITY_PENDING";

m_memory.LastSnapshot.OBRange =
   m_memory.OB.High - m_memory.OB.Low;

m_memory.LastSnapshot.OBStrength =
   (int)m_memory.OB.Strength;
   ...
   m_memory.Trade.TradeTicket;
      ...
      // STEP129C - Entry Quality Basic Evaluation
if(m_memory.Execution.AuditReason == "EXECUTION_ALLOWED")
{
   m_memory.LastSnapshot.EntryQualityScore = 0.0;
   m_memory.LastSnapshot.EntryQualityReason = "";

   if(m_memory.OB.Valid)
      m_memory.LastSnapshot.EntryQualityScore += 30.0;
   else
      m_memory.LastSnapshot.EntryQualityReason += "NO_VALID_OB;";

   if(m_memory.Execution.PermissionScore >= 60.0)
      m_memory.LastSnapshot.EntryQualityScore += 30.0;
   else
      m_memory.LastSnapshot.EntryQualityReason += "LOW_PERMISSION_SCORE;";

   if(m_memory.Execution.ConfidenceLevel == "MEDIUM" ||
      m_memory.Execution.ConfidenceLevel == "HIGH")
      m_memory.LastSnapshot.EntryQualityScore += 20.0;
   else
      m_memory.LastSnapshot.EntryQualityReason += "LOW_CONFIDENCE;";

   if(m_memory.Execution.ExecutionGrade == "A_SETUP" ||
      m_memory.Execution.ExecutionGrade == "B_SETUP")
      m_memory.LastSnapshot.EntryQualityScore += 20.0;
   else
      m_memory.LastSnapshot.EntryQualityReason += "WEAK_SETUP_GRADE;";

   if(m_memory.LastSnapshot.EntryQualityScore >= 80.0)
      m_memory.LastSnapshot.EntryQualityClass = "HIGH_ENTRY_QUALITY";
   else if(m_memory.LastSnapshot.EntryQualityScore >= 60.0)
      m_memory.LastSnapshot.EntryQualityClass = "MEDIUM_ENTRY_QUALITY";
   else if(m_memory.LastSnapshot.EntryQualityScore > 0.0)
      m_memory.LastSnapshot.EntryQualityClass = "LOW_ENTRY_QUALITY";
   else
      m_memory.LastSnapshot.EntryQualityClass = "NO_ENTRY_QUALITY";

   if(m_memory.LastSnapshot.EntryQualityReason == "")
      m_memory.LastSnapshot.EntryQualityReason = "ENTRY_QUALITY_ACCEPTABLE";
}
   
   MRH_Log("ML_DATASET_ENGINE",
        "STEP129_ENTRY_QUALITY",
        "Score=" + DoubleToString(m_memory.LastSnapshot.EntryQualityScore, 1) +
        " | Class=" + m_memory.LastSnapshot.EntryQualityClass +
        " | Reason=" + m_memory.LastSnapshot.EntryQualityReason);
...
// STEP121 - Execution Bottleneck Analytics Reset
m_memory.LastSnapshot.ExecutionBlockedByStructure = 0;
m_memory.LastSnapshot.ExecutionBlockedByOB = 0;
m_memory.LastSnapshot.ExecutionBlockedByLiquidity = 0;
m_memory.LastSnapshot.ExecutionBlockedByPermission = 0;
m_memory.LastSnapshot.ExecutionAllowedCount = 0;



// STEP121 - Execution Bottleneck Analytics Classification
if(m_memory.LastSnapshot.PrimaryBlockReason == "NO_STRUCTURE_BIAS" ||
   m_memory.LastSnapshot.PrimaryBlockReason == "STRUCTURE_RANGE")
{
   m_memory.LastSnapshot.ExecutionBlockedByStructure = 1;
}
else if(m_memory.LastSnapshot.PrimaryBlockReason == "NO_VALID_OB" ||
        m_memory.LastSnapshot.PrimaryBlockReason == "OB_INVALIDATED")
{
   m_memory.LastSnapshot.ExecutionBlockedByOB = 1;
}
else if(m_memory.LastSnapshot.PrimaryBlockReason == "NO_TARGET_LIQUIDITY")
{
   m_memory.LastSnapshot.ExecutionBlockedByLiquidity = 1;
}
else if(m_memory.LastSnapshot.PrimaryBlockReason == "LOW_PERMISSION_SCORE")
{
   m_memory.LastSnapshot.ExecutionBlockedByPermission = 1;
}
else if(m_memory.Execution.AuditReason == "EXECUTION_ALLOWED")
{
   m_memory.LastSnapshot.ExecutionAllowedCount = 1;
}


// STEP122 - Persistent Execution Bottleneck Statistics Update
m_bottleneckTotalSamples++;

if(m_memory.LastSnapshot.ExecutionBlockedByStructure == 1)
   m_bottleneckStructureCount++;

if(m_memory.LastSnapshot.ExecutionBlockedByOB == 1)
   m_bottleneckOBCount++;

if(m_memory.LastSnapshot.ExecutionBlockedByLiquidity == 1)
   m_bottleneckLiquidityCount++;

if(m_memory.LastSnapshot.ExecutionBlockedByPermission == 1)
   m_bottleneckPermissionCount++;

if(m_memory.LastSnapshot.ExecutionAllowedCount == 1)
   m_executionAllowedCount++;

// STEP123 - Root Cause Analytics Update
if(m_memory.LastSnapshot.PrimaryBlockReason == "NO_STRUCTURE_BIAS")
   m_rootNoStructureBiasCount++;

else if(m_memory.LastSnapshot.PrimaryBlockReason == "STRUCTURE_RANGE")
   m_rootStructureRangeCount++;

else if(m_memory.LastSnapshot.PrimaryBlockReason == "NO_VALID_OB")
   m_rootNoValidOBCount++;

else if(m_memory.LastSnapshot.PrimaryBlockReason == "OB_INVALIDATED")
   m_rootOBInvalidatedCount++;

// STEP123D - Unmatched Root Cause Debug
if(m_memory.LastSnapshot.ExecutionBlockedByStructure == 1 &&
   m_memory.Execution.AuditReason != "NO_STRUCTURE_BIAS" &&
   m_memory.Execution.AuditReason != "STRUCTURE_RANGE")
{
   MRH_Log("ML_DATASET_ENGINE",
           "STEP123D_UNMATCHED_STRUCTURE_ROOT",
           "AuditReason=" + m_memory.Execution.AuditReason);
}

// STEP122 - Execution Bottleneck Statistics Summary Log
if(m_bottleneckTotalSamples > 0)
{
   double structureRate =
      (100.0 * m_bottleneckStructureCount) / m_bottleneckTotalSamples;

   double obRate =
      (100.0 * m_bottleneckOBCount) / m_bottleneckTotalSamples;

   double liquidityRate =
      (100.0 * m_bottleneckLiquidityCount) / m_bottleneckTotalSamples;

   double permissionRate =
      (100.0 * m_bottleneckPermissionCount) / m_bottleneckTotalSamples;

   double allowedRate =
      (100.0 * m_executionAllowedCount) / m_bottleneckTotalSamples;

   MRH_Log("ML_DATASET_ENGINE",
           "STEP122_BOTTLENECK_STATS",
           "Samples=" + IntegerToString(m_bottleneckTotalSamples) +
           " | Structure=" + IntegerToString(m_bottleneckStructureCount) +
           " (" + DoubleToString(structureRate, 2) + "%)" +
           " | OB=" + IntegerToString(m_bottleneckOBCount) +
           " (" + DoubleToString(obRate, 2) + "%)" +
           " | Liquidity=" + IntegerToString(m_bottleneckLiquidityCount) +
           " (" + DoubleToString(liquidityRate, 2) + "%)" +
           " | Permission=" + IntegerToString(m_bottleneckPermissionCount) +
           " (" + DoubleToString(permissionRate, 2) + "%)" +
           " | Allowed=" + IntegerToString(m_executionAllowedCount) +
           " (" + DoubleToString(allowedRate, 2) + "%)" +
           " | CurrentAuditReason=" + m_memory.Execution.AuditReason);
           
           // STEP123 - Root Cause Analytics Summary Log
MRH_Log("ML_DATASET_ENGINE",
        "STEP123_ROOT_CAUSE_STATS",
        "NO_STRUCTURE_BIAS=" + IntegerToString(m_rootNoStructureBiasCount) +
        " | STRUCTURE_RANGE=" + IntegerToString(m_rootStructureRangeCount) +
        " | NO_VALID_OB=" + IntegerToString(m_rootNoValidOBCount) +
        " | OB_INVALIDATED=" + IntegerToString(m_rootOBInvalidatedCount));
}
...
if(!IsDatasetRowComplete())
{
   ...
   (m_memory.LastSnapshot.OBValid ? "TRUE" : "FALSE") + "\t" +
   IntegerToString(m_memory.LastSnapshot.OBFreshness) + "\t" +
   (m_memory.LastSnapshot.OBMitigated ? "TRUE" : "FALSE") + "\t" +
   (m_memory.LastSnapshot.OBInvalidated ? "TRUE" : "FALSE") + "\t" +
   DoubleToString(m_memory.LastSnapshot.OBRange, _Digits) + "\t" +
   IntegerToString(m_memory.LastSnapshot.OBStrength) + "\t" +
...
DoubleToString(m_memory.LastSnapshot.DemoReadinessScore, 2) + "\t" +
m_memory.LastSnapshot.DemoReadinessClass + "\t" +
(m_memory.LastSnapshot.DemoReady ? "TRUE" : "FALSE") + "\t" +

m_memory.LastSnapshot.DemoReadinessReason + "\t" +
(m_memory.LastSnapshot.DemoGateOpen ? "TRUE" : "FALSE") + "\t" +
m_memory.LastSnapshot.DemoGateStatus + "\t" +

m_memory.LastSnapshot.DemoGateReason + "\t" +
(m_memory.LastSnapshot.DemoObservationMode ? "TRUE" : "FALSE") + "\t" +
m_memory.LastSnapshot.DemoObservationStatus + "\t" +

m_memory.LastSnapshot.DemoObservationReason + "\t" +
m_memory.LastSnapshot.DemoGateBlockers + "\t" +
IntegerToString(m_memory.LastSnapshot.DemoGateFailedConditions) + "\t" +

(m_memory.LastSnapshot.DemoGateDiagnosticsReady ? "TRUE" : "FALSE") + "\t" +
DoubleToString(m_memory.LastSnapshot.ValidationHistoryAverage, 2) + "\t" +
DoubleToString(m_memory.LastSnapshot.ValidationHistoryMinimum, 2) + "\t" +
DoubleToString(m_memory.LastSnapshot.ValidationHistoryMaximum, 2) + "\t" +
IntegerToString(m_memory.LastSnapshot.ValidationHistorySamples) + "\t" +
IntegerToString(m_memory.LastSnapshot.ExecutionBlockedByStructure) + "\t" +
IntegerToString(m_memory.LastSnapshot.ExecutionBlockedByOB) + "\t" +
IntegerToString(m_memory.LastSnapshot.ExecutionBlockedByLiquidity) + "\t" +
IntegerToString(m_memory.LastSnapshot.ExecutionBlockedByPermission) + "\t" +
IntegerToString(m_memory.LastSnapshot.ExecutionAllowedCount) + "\t" +
m_memory.LastSnapshot.PrimaryBlockReason + "\t" +
DoubleToString(m_memory.LastSnapshot.EntryQualityScore,1) + "\t" +
m_memory.LastSnapshot.EntryQualityClass + "\t" +
m_memory.LastSnapshot.EntryQualityReason;
...
double probabilityBaseScore =
   (m_memory.LastSnapshot.HistoricalSetupConfidence * 0.30) +
   (m_memory.LastSnapshot.FeatureReliabilityScore * 0.20) +
   (m_memory.LastSnapshot.AdaptiveFeatureWeightScore * 0.20) +
   (m_memory.LastSnapshot.HistoricalFeaturePerformanceScore * 0.30);

m_memory.LastSnapshot.ProbabilityConfidence = probabilityBaseScore;

if(probabilityBaseScore >= 70.0)
{
   m_memory.LastSnapshot.ProbabilityClass = "HIGH_PROBABILITY";
   m_memory.LastSnapshot.ProbabilityReady = true;
   m_memory.LastSnapshot.ProbabilityReason = "PROBABILITY_ENGINE_READY";
}
else if(probabilityBaseScore >= 50.0)
{
   m_memory.LastSnapshot.ProbabilityClass = "MEDIUM_PROBABILITY";
   m_memory.LastSnapshot.ProbabilityReady = false;
   m_memory.LastSnapshot.ProbabilityReason = "PROBABILITY_ENGINE_PARTIAL";
}
else
{
   m_memory.LastSnapshot.ProbabilityClass = "LOW_PROBABILITY";
   m_memory.LastSnapshot.ProbabilityReady = false;
   m_memory.LastSnapshot.ProbabilityReason = "PROBABILITY_ENGINE_WEAK";
}

MRH_Log("ML_DATASET_ENGINE",
        "STEP111_PROBABILITY_SCORE",
        "Confidence=" + DoubleToString(m_memory.LastSnapshot.ProbabilityConfidence, 2) +
   ...
   m_memory.LastSnapshot.ProbabilityCalibrationReason = "HIGH_CALIBRATION_ERROR";
    ...
    MRH_Log("ML_DATASET_ENGINE",
        ...
        " | Reason=" + m_memory.LastSnapshot.ProbabilityCalibrationReason);
                    ...
                    // STEP113 - Demo Readiness Evaluation Engine

double demoReadinessScore =
      (m_memory.LastSnapshot.HistoricalSetupConfidence * 0.20) +
      (m_memory.LastSnapshot.FeatureReliabilityScore * 0.20) +
      (m_memory.LastSnapshot.HistoricalFeaturePerformanceScore * 0.20) +
      (m_memory.LastSnapshot.ProbabilityConfidence * 0.20) +
      (m_memory.LastSnapshot.ProbabilityCalibrationScore * 0.20);

m_memory.LastSnapshot.DemoReadinessScore = demoReadinessScore;

if(demoReadinessScore >= 80.0)
{
   m_memory.LastSnapshot.DemoReadinessClass = "READY_FOR_DEMO";
   m_memory.LastSnapshot.DemoReady = true;
   m_memory.LastSnapshot.DemoReadinessReason = "ALL_CORE_VALIDATIONS_PASSED";
}
else if(demoReadinessScore >= 60.0)
{
   m_memory.LastSnapshot.DemoReadinessClass = "PARTIALLY_READY";
   m_memory.LastSnapshot.DemoReady = false;
   m_memory.LastSnapshot.DemoReadinessReason = "MORE_RUNTIME_EVIDENCE_REQUIRED";
}
else
{
   m_memory.LastSnapshot.DemoReadinessClass = "NOT_READY_FOR_DEMO";
   m_memory.LastSnapshot.DemoReady = false;
   m_memory.LastSnapshot.DemoReadinessReason = "INSUFFICIENT_STATISTICAL_CONFIDENCE";
}
  
  MRH_Log("ML_DATASET_ENGINE",
        "STEP113_DEMO_READINESS",
        "Score=" + DoubleToString(m_memory.LastSnapshot.DemoReadinessScore, 2) +
        " | Class=" + m_memory.LastSnapshot.DemoReadinessClass +
        " | Ready=" + (m_memory.LastSnapshot.DemoReady ? "TRUE" : "FALSE") +
        " | Reason=" + m_memory.LastSnapshot.DemoReadinessReason);
        
          // STEP114 - Demo Decision Gate
int minimumDemoTrades = 30;

if(m_memory.LastSnapshot.HistoricalFeatureTrades >= minimumDemoTrades &&
   m_memory.LastSnapshot.ProbabilityCalibrationReady &&
   m_memory.LastSnapshot.ProbabilityReady &&
   m_memory.LastSnapshot.FeatureReliabilityReady &&
   m_memory.LastSnapshot.HistoricalFeaturePerformanceReady)
{
   m_memory.LastSnapshot.DemoGateOpen = true;
   m_memory.LastSnapshot.DemoGateStatus = "DEMO_GATE_OPEN";
   m_memory.LastSnapshot.DemoGateReason = "ALL_DEMO_GATE_CONDITIONS_PASSED";
}
else
{
   m_memory.LastSnapshot.DemoGateOpen = false;
   m_memory.LastSnapshot.DemoGateStatus = "DEMO_GATE_CLOSED";
   m_memory.LastSnapshot.DemoGateReason = "DEMO_GATE_CONDITIONS_NOT_MET";
}

MRH_Log("ML_DATASET_ENGINE",
        "STEP114_DEMO_DECISION_GATE",
        "Gate=" + m_memory.LastSnapshot.DemoGateStatus +
        " | Open=" + (m_memory.LastSnapshot.DemoGateOpen ? "TRUE" : "FALSE") +
        " | Reason=" + m_memory.LastSnapshot.DemoGateReason +
        " | Trades=" + IntegerToString(m_memory.LastSnapshot.HistoricalFeatureTrades));
        
                          // STEP115 - Controlled Demo Observation Layer

m_memory.LastSnapshot.DemoObservationMode = true;

if(m_memory.LastSnapshot.DemoGateOpen)
{
   m_memory.LastSnapshot.DemoObservationStatus = "OBSERVATION_GATE_OPEN";
   m_memory.LastSnapshot.DemoObservationReason = "MONITORING_REAL_DEMO_BEHAVIOR";
}
else if(m_memory.LastSnapshot.HistoricalFeatureTrades < 10)
{
   m_memory.LastSnapshot.DemoObservationStatus = "OBSERVATION_BUILDING_SAMPLE";
   m_memory.LastSnapshot.DemoObservationReason = "WAITING_FIRST_10_TRADES";
}
else
{
   m_memory.LastSnapshot.DemoObservationStatus = "OBSERVATION_GATE_CLOSED";
   m_memory.LastSnapshot.DemoObservationReason = "CONTINUE_DATA_COLLECTION";
}
                  MRH_Log("ML_DATASET_ENGINE",
        "STEP115_DEMO_OBSERVATION",
        "Mode=" + (m_memory.LastSnapshot.DemoObservationMode ? "TRUE" : "FALSE") +
        " | Status=" + m_memory.LastSnapshot.DemoObservationStatus +
        " | Reason=" + m_memory.LastSnapshot.DemoObservationReason +
        " | Trades=" + IntegerToString(m_memory.LastSnapshot.HistoricalFeatureTrades));
        
        // STEP116 - Demo Gate Diagnostics Engine

m_memory.LastSnapshot.DemoGateBlockers = "";
m_memory.LastSnapshot.DemoGateFailedConditions = 0;

if(m_memory.LastSnapshot.HistoricalFeatureTrades < minimumDemoTrades)
{
   m_memory.LastSnapshot.DemoGateBlockers += "MIN_TRADES;";
   m_memory.LastSnapshot.DemoGateFailedConditions++;
}

if(!m_memory.LastSnapshot.ProbabilityReady)
{
   m_memory.LastSnapshot.DemoGateBlockers += "PROBABILITY;";
   m_memory.LastSnapshot.DemoGateFailedConditions++;
}

if(!m_memory.LastSnapshot.ProbabilityCalibrationReady)
{
   m_memory.LastSnapshot.DemoGateBlockers += "CALIBRATION;";
   m_memory.LastSnapshot.DemoGateFailedConditions++;
}

if(!m_memory.LastSnapshot.FeatureReliabilityReady)
{
   m_memory.LastSnapshot.DemoGateBlockers += "FEATURE_RELIABILITY;";
   m_memory.LastSnapshot.DemoGateFailedConditions++;
}

if(!m_memory.LastSnapshot.HistoricalFeaturePerformanceReady)
{
   m_memory.LastSnapshot.DemoGateBlockers += "FEATURE_HISTORY;";
   m_memory.LastSnapshot.DemoGateFailedConditions++;
}

m_memory.LastSnapshot.DemoGateDiagnosticsReady =
   (m_memory.LastSnapshot.DemoGateFailedConditions == 0);
   
   MRH_Log("ML_DATASET_ENGINE",
        "STEP116_DEMO_GATE_DIAGNOSTICS",
...
double validationHistoryValue =
   m_memory.LastSnapshot.DemoReadinessScore;

m_validationHistorySum += validationHistoryValue;
m_validationHistorySamples++;

if(m_validationHistorySamples == 1)
{
   m_validationHistoryMin = validationHistoryValue;
   m_validationHistoryMax = validationHistoryValue;
}
else
{
   if(validationHistoryValue < m_validationHistoryMin)
      m_validationHistoryMin = validationHistoryValue;

   if(validationHistoryValue > m_validationHistoryMax)
      m_validationHistoryMax = validationHistoryValue;
}

m_memory.LastSnapshot.ValidationHistorySamples =
   m_validationHistorySamples;

m_memory.LastSnapshot.ValidationHistoryAverage =
   m_validationHistorySum / m_validationHistorySamples;

m_memory.LastSnapshot.ValidationHistoryMinimum =
   m_validationHistoryMin;

m_memory.LastSnapshot.ValidationHistoryMaximum =
   m_validationHistoryMax;
                          
                      MRH_Log("ML_DATASET_ENGINE",
        "STEP118_VALIDATION_HISTORY",
        "Samples=" + IntegerToString(m_memory.LastSnapshot.ValidationHistorySamples) +
        " | Avg=" + DoubleToString(m_memory.LastSnapshot.ValidationHistoryAverage, 2) +
        " | Min=" + DoubleToString(m_memory.LastSnapshot.ValidationHistoryMinimum, 2) +
        " | Max=" + DoubleToString(m_memory.LastSnapshot.ValidationHistoryMaximum, 2));    
